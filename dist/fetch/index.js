...
const fetchStream = (source, opt = {}, raw = false) => {
  ...
  const getStream = setupResult => {
    ...
    const nextPageParser = source.pagination.nextPageSelector ? (0, _parse.default)(source.parser, {
      ...source.parserOptions,
      selector: source.pagination.nextPageSelector
    }) : null;
    const parser = nextPageParser ? createParser(baseParser, nextPageParser) : baseParser;
